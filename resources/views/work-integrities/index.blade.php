<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-md text-gray-800 dark:text-gray-200 leading-tight">
                Integridad Laboral - Resumen por Persona
            </h2>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    @php
                        ...
                        if (auth()->user()->can('work-integrities.index')) {
                        ...
                        if (auth()->user()->can('work-integrities.create')) {
    ...
    <script>
        function viewIntegrations(item) {
            ...
            window.location.href = `{{ url('work-integrities/person') }}/${personId}`;
        }

        function createWorkIntegrity(item) {
            const personId = item.id || item;
            window.location.href = `{{ url('work-integrities/create') }}?person_id=${personId}`;
        }
    </script>
</x-app-layout>
