    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-md text-gray-800 dark:text-gray-200 leading-tight">
                Mi Perfil - {{ $person->name }} {{ $person->last_name }}
            </h2>
            <div class="flex space-x-2">
                <a href="{{ route('user.dashboard') }}" class="bg-gray-600 hover:bg-gray-700 text-white font-medium py-1 px-3 rounded-lg transition-colors duration-200">
                    Volver al Dashboard
                </a>
            </div>
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div x-show="activeTab === 'personal'">
                        <form action="{{ route('user.profile.update-personal-info') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                    ...
                    <div x-show="activeTab === 'residence'">
                        <form action="{{ route('user.profile.update-residence-info') }}" method="POST">
                            @csrf
                            @method('PATCH')
                        ...
                        <script>
                            ...
                            async function cargarDistritos(municipalityId, selectedDistrictId = null) {
                                ...
                                try {
                                    const response = await fetch(`{{ route('user.districts-by-municipality') }}?municipality_id=${municipalityId}`);
                    ...
                    <div x-show="activeTab === 'educational'" class="space-y-6">
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                            ...
                            <form action="{{ route('user.profile.educational-skills.store') }}" method="POST" class="space-y-1">
                                @csrf
                                            ...
                                            @foreach($person->educationalSkills as $skill)
                                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                        <form action="{{ route('user.profile.educational-skills.destroy', $skill) }}" method="POST" class="inline-block delete-skill-form" data-skill-name="{{ $skill->career }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button 
                                                                type="submit"
                                        ...
                                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                            @foreach($person->workExperiences as $experience)
                                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                        <form action="{{ route('user.profile.work-experiences.destroy', $experience) }}" method="POST" class="inline-block delete-experience-form" data-company-name="{{ $experience->company_name }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button 
                                                                type="submit"
                                        ...
                                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                            @foreach($person->personalReferences as $reference)
                                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                        <form action="{{ route('user.profile.personal-references.destroy', $reference) }}" method="POST" class="inline-block delete-reference-form" data-reference-name="{{ $reference->full_name }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button 
                                                                type="submit"
                    ...
                    <div x-show="activeTab === 'aspirations'">
                        <form action="{{ route('user.profile.update-aspiration') }}" method="POST">
                            @csrf
                            @method('PATCH')
